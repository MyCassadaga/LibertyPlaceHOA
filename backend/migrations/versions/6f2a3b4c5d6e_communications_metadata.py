...
def upgrade() -> None:
    with op.batch_alter_table("announcements") as batch_op:
        batch_op.add_column(sa.Column("recipient_snapshot", sa.JSON(), nullable=False, server_default=sa.text("'[]'")))
    ...
    with op.batch_alter_table("email_broadcasts") as batch_op:
        batch_op.add_column(sa.Column("delivery_methods", sa.JSON(), nullable=False, server_default=sa.text("'[\"email\"]'")))
        batch_op.add_column(sa.Column("sender_snapshot", sa.JSON(), nullable=True))
